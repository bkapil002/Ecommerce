...
const UploadProduct = ({ onClose ,fetchdata }) => {
  ...
  const headerSubmit = async (e) => {
    e.preventDefault();
    try {
      const dataResponse = await fetch(summaryApi.uplodeProduct.url, {
        method: summaryApi.updatedata.method,
        credentials: "include",
        headers: {
  ...
  return (
    <div className='fixed top-0 left-0 right-0 bottom-0 flex justify-center items-center bg-sliate-200 bg-opacity-10'>
        ...
        <label htmlFor='productName'> Name</label>
          ...
          <select 
                ...
                {catagorys.map((el, index) => (
                 <option value={el.value} key={el.value + index} disabled={index === 0}>{el.label}</option>
                     ))}
          </select>
      ...
      {fullOpenImage && (
        <DisplayImage onClose={() => setfullOpenImage(false)} imgeUrl={fullScreen} fetchdata={fetchdata}/>
      )}
    </div>
  );
}
